...
function show_comments($comments, $id = -1)
{
    $html = '';
    if ($id != -1) {
...
function show_write_comment_form($id = -1)
{
    $html = '
    <div class="write_comment" data-comment-id="' . $id . '">
        <form>
            <input name="id" type="hidden" value="' . $id . '">
            <textarea name="content" placeholder="Write your comment here..." required></textarea>
            <button type="submit" onclick="location.reload()">Submit Comment</button>
...
if (isset($_GET['page_id'])) {
    ...
    if (isset($_POST['content'])) {
        ...
        $stmt = $pdo->prepare('INSERT INTO comments (page_id, id, name, content, submit_date) VALUES (?,?,?,?,NOW())');
        $stmt->execute([$_GET['page_id'], $_POST['id'], $_SESSION['parentcrn_brn_no'], $_POST['content']]);
